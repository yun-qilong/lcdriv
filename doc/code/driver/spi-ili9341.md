...
> **状态：可约束实现。** 本文是 Driver 层（组合根 + CS 事务 + 门面）的算法规范。数据来源：Bus spec（Bus）、Controller spec（Controller）；协议数据经硬件验证。
...
- M/N = **逻辑分辨率**（应用帧缓冲列/行，Controller spec §4.1）；方向（MADCTL）由 M/N 编译期推导（Controller spec §4.2），当前项目目标为横屏（M > N 实例）。
...
public:
    ...
    // 生命周期：默认构造 + 双范式初始化；不可拷贝（拷贝 = 复制引脚/句柄所有权，
    // 事务归属不明），支持移动（见 §3）
    LcdDriver() = default;
    ...
    bool init(SPI_HandleTypeDef* spi,                     // SPI 句柄（使用方已初始化）
...
- 两种范式使用同一套装配逻辑与门面接口：`init(...)` 与 `LcdDriver(...)` 行为等价（读入句柄 + 上电序列 Controller spec §6），区别只在"何时触发"。

> 类型说明：`SPI_HandleTypeDef`/`GPIO_TypeDef` 为 STM32 HAL/CMSIS 类型（定义于使用方工程的 `Drivers/`，STM32Cube，非本库定义，typedef 匿名结构体无法前向声明；类型名在 STM32 全家族一致）。库头文件**不 include 家族头**（避免锁死具体系列），采用 **include 顺序契约**：使用方 TU 先 include 工程 `main.h` 再 include 伞头 `lcdriv.hpp`（详见 Bus spec §3）。入参的绑定面是"STM32（任一家族）HAL"；非 STM32 平台才需改这些参数类型（模板参数仍为通用化轴）。
...
- **无硬件自检**（`init` 恒返回 true 是可接受的实现；`bool` 为将来错误路径预留）；接线/型号验证靠 `readID` 由调用方比对。
...
- 入参有效：`spi` 非空；引脚已由使用方配置为输出；调用方保证 HAL（SPI/GPIO）已初始化。
...
- 推论：`pushFrame` 的 153,600 字节像素流（多次 `bus.send`）与上电的长延时（≈395ms）都发生在 CS 有效期间——ILI9341 允许（CS 为纯使能；实测驱动为命令级 CS，与本策略字节协议等价，均已验证可用）。
...
- **功能**：完成一次完整上电初始化。读入句柄：装配总线（SPI 句柄）与控制器（DC/RST 引脚）、保存 CS 引脚并置空闲高；随后在单事务内执行上电序列（硬件复位 + Controller spec §6 命令表，阻塞总耗时 ≈395ms）。
- `init` 返回 `true`；`LcdDriver(...)` 构造完成后对象即处于同一状态。
- **前置**：HAL 已初始化；入参有效（见 §3）。
- **后置**：屏幕可显示（若接线正确）；MADCTL 已按 M/N 推导（Controller spec §4.2）。
- **边界**：只支持 SPI + ILI9341；其他组合编译期报错。`spi == nullptr` 等非法输入未定义（无入参校验）。

### `void pushFrame(const uint8_t* px)`

- **功能**：单事务内推送一整帧。事务序列 = 0x2A 全屏列窗口（应用空间 0..M-1）→ 0x2B 全屏行窗口（0..N-1）→ 0x2C + `M*N*2` 字节像素（协议细节、分块与终止语义见 Controller spec `pushFrame`）。
...
- **功能**：单事务内整屏填充单色（Controller spec `fillScreen`）。
- **前置**：已完成装配与上电。
- **后置**：所有像素变为 `color`。

### `uint32_t readID()`

- **功能**：单事务内读芯片 ID（Controller spec `readID` 协议），返回组合值（ILI9341 期望 `0x9341??`，实测 `0x934100`）。
- **前置**：已完成装配与上电；MISO 已接。
- **校验**：驱动不校验，由调用方比对 `0x9341` 段。

### `void setOrientation(uint8_t madctl)`

- **功能**：单事务内写 0x36（MADCTL）（Controller spec `setOrientation`）。
- **注意**：正常使用不调用（方向已由模板参数在装配期推导）；本函数仅运行时覆盖/调试用。

### `int width() const` / `int height() const`

- **功能**：返回编译期逻辑分辨率常量 M / N（如横屏实例 M=320、N=240）。
- **说明**：与应用帧缓冲一致（Controller spec §4）。
...
| CS | Driver（本类） | `cs_port` / `cs_pin` |
| DC | Controller | `dc_port` / `dc_pin` |
| RST | Controller | `rst_port` / `rst_pin` |

> 引脚号由使用方接线并在构造时注入（Driver/Controller 构造入参），本库不绑定任何具体 GPIO。

## 7. 当前限制

- 无错误路径（假定硬件正常）；无背光控制；阻塞传输（DMA 属 Bus 层后续，本类接口与事务语义不变）。

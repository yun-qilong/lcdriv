...
│   │   └── ili9341.md        #     Controller<ILI9341, M, N>
│   └── driver/               #   Driver 层各搭配算法 spec（新搭配 = 加文件）
│       └── spi-ili9341.md    #     LcdDriver<SPI, ILI9341, M, N>
└── test/                     # 测试 spec（怎么验证）
...
2. 自下而上读算法 spec：`code/bus/spi.md` → `code/controller/ili9341.md` → `code/driver/spi-ili9341.md`。
...
- **跨文档引用用层短名**（Bus spec / Controller spec / Driver spec / 架构 / 测试框架），不写文件路径——文件搬迁不破坏引用。
